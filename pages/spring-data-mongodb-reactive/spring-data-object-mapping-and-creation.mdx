...
- id 필드가 없으면 내부적으로 리플렉션을 통해 id 필드를생성
- id 필드에 `@Id` 가 안붙어있어도 필드명이 id 이면서 ObjectId 타입일 경우 `_id` 필드로 인식
- 인자가 없는 기본생성자 무조건 하나 있어야 합니다.
- Spring Data MongoDB Reactive 에서는 기본생성자가 private 접근 레벨로 정의되는 것이 허용됩니다.
...
- Spring Data MongoDB Reactive 에서 BigDecimal 을 사용할 경우 별도로 BigDecimal → Decimal128, Decimal128 → BigDecimal 로 Converting 을 하는 Converter 를 별도로 작성해주셔야 합니다. 제일 아래에 BigDecimal Converter 변환방법 섹션을 참고해주세요.
...
팩토리 역할의 컴포넌트를 생성하면 객체 생성의 테스트가 분담되기에 엔티티 코드에 비즈니스 내용이 간섭을 하지 않으며, 변경사항에 유연하게 대응이 가능하며, 메서드나 객체 명을 따로 지정해 용도별로 분리가 가능합니다.<br/>
