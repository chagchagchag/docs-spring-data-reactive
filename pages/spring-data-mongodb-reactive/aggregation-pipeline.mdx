...
이번 예제에서 사용할 샘플 데이터는 아래와 같습니다. helloworld 라는 데이터베이스가 있고 이 데이터베이스 안의 `book` 이라는 컬렉션을 예제로 사용합니다. 글고 데이터를 자세히 보면, "바람과 함께 사라지다" 라는 책이 다양한 종류로 존재하며 가격은 10000, 15000, 20000, 25000, 30000 원대가 있으며, 판매 상태는 `SOLD_OUT`, `FOR_SALE` 이 존재함을 알 수 있습니다.<br/>
